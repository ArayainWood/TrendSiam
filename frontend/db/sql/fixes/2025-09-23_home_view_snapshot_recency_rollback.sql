-- =========================================================
-- ROLLBACK: HOME VIEW SNAPSHOT-BASED FRESHNESS
-- Date: 2025-09-23
--
-- This rollback script restores the public_v_home_news view
-- to its baseline state (date-based freshness) if needed.
--
-- To execute rollback:
-- psql -h <host> -U <user> -d <database> -f 2025-09-23_home_view_snapshot_recency_rollback.sql
-- =========================================================

BEGIN;

\echo '========================================='
\echo 'ROLLBACK: Snapshot-based freshness'
\echo 'Restoring baseline view definition...'
\echo '========================================='
...
-- =========================================
-- 2. RESTORE BASELINE VIEW (Date-based)
-- =========================================
...
-- =========================================
-- 3. RESTORE SECURITY AND PERMISSIONS
-- =========================================
...
-- Update freshness policy back to date-based
UPDATE system_meta 
SET 
  value = 'date_based:48h_primary|7d_fallback',
  description = 'Home feed freshness policy: uses publish date within 48h (primary) or 7d (fallback)',
  updated_at = NOW()
...
UPDATE system_meta 
SET 
  value = (SELECT columns_hash FROM column_list),
  updated_at = NOW()
...
ROLLED BACK from snapshot-based to date-based freshness: 2025-09-23';
...
BEGIN
  ...
  RAISE NOTICE 'View has % rows after rollback', v_count;
  ...
  IF v_policy = 'date_based:48h_primary|7d_fallback' THEN
    RAISE NOTICE '✅ Successfully rolled back to date-based freshness';
  ELSE
    RAISE WARNING '⚠️  Unexpected policy after rollback: %', v_policy;
  END IF;
END $$;

\echo '========================================='
\echo 'ROLLBACK COMPLETE'
\echo 'View restored to date-based freshness'
\echo '========================================='

COMMIT;

-- Note: The indexes created in the forward migration are kept
-- as they don't affect functionality and may improve performance
