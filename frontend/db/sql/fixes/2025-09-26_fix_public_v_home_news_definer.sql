-- =========================================================
-- FIX HOME VIEW TO USE PUBLIC_V_SYSTEM_META
-- Date: 2025-09-26
--
-- Fixes "permission denied for table system_meta" by:
-- 1. Replacing direct system_meta access with public_v_system_meta
-- 2. Maintaining definer semantics for security
-- 3. Preserving exact 26-column contract
-- =========================================================

BEGIN;

-- =========================================
-- DROP AND RECREATE WITH PROPER DEPENDENCIES
-- =========================================

DROP VIEW IF EXISTS public.public_v_home_news CASCADE;

-- Create view using public_v_system_meta instead of system_meta
CREATE VIEW public.public_v_home_news 
WITH (security_invoker = false, security_barrier = true) AS
WITH cfg AS (
  -- Read configuration from PUBLIC VIEW, not base table
  SELECT
    COALESCE((SELECT value::int FROM public.public_v_system_meta WHERE key='home_limit'), 20) AS home_limit,
    COALESCE((SELECT value::int FROM public.public_v_system_meta WHERE key='top3_max'), 3) AS top3_max
),
...
-- =========================================
-- CREATE FALLBACK RPC FUNCTION (IF NEEDED)
-- =========================================

-- Create SECURITY DEFINER function as ultimate fallback
CREATE OR REPLACE FUNCTION public.get_public_home_news(
...
-- Set function owner
ALTER FUNCTION public.get_public_home_news(INTEGER, INTEGER) OWNER TO postgres;

-- Grant execute permissions
REVOKE ALL ON FUNCTION public.get_public_home_news(INTEGER, INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_public_home_news(INTEGER, INTEGER) TO anon, authenticated;
...
Security: DEFINER view with no direct base table references.
Uses public_v_system_meta for config values (not system_meta).
...
Updated: 2025-09-26 - Fixed to use public_v_system_meta instead of system_meta';

COMMENT ON FUNCTION public.get_public_home_news(INTEGER, INTEGER) IS
'Fallback RPC function for home news access if view permissions fail.
Returns same data as public_v_home_news view with pagination.
...
BEGIN
  ...
  BEGIN
    ...
    RAISE NOTICE '✅ View accessible, % rows found', v_count;
  EXCEPTION WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS v_error = MESSAGE_TEXT;
    RAISE WARNING '❌ View not accessible: %', v_error;
  END;
  
  -- Test that view no longer references system_meta directly
  IF EXISTS (
    SELECT 1 
    FROM pg_views 
    ...
    RAISE WARNING '❌ View still references system_meta base table!';
  ELSE
    RAISE NOTICE '✅ View correctly uses public_v_system_meta';
  END IF;
  ...
  -- Test RPC function
  BEGIN
    SELECT COUNT(*) INTO v_count FROM public.get_public_home_news(5, 0);
    RAISE NOTICE '✅ RPC function works, returned % rows', v_count;
  EXCEPTION WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS v_error = MESSAGE_TEXT;
