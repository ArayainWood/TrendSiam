-- ===== TYPE-SAFE RENAME-SWAP FIX (2025-09-27) =====
-- Fix 42883 (text >= numeric), proper type casting, maintain 26-column contract
...
-- Step 4: Create main home view with correct joins and type casting (26 columns)
...
joined_data as (
  select
    ...
    -- Views/likes/comments from snapshots (safe integer casting)
    case 
      when snap.view_count ~ '^\d+$' then snap.view_count::bigint
      else null
    end as views,
    case 
      when snap.like_count ~ '^\d+$' then snap.like_count::bigint
      else null
    end as likes,
    case 
      when snap.comment_count ~ '^\d+$' then snap.comment_count::bigint
      else null
    end as comments,
...
select
  -- Exactly 26 columns in the required order with proper type casting
  id::text,                                              -- 1. id
  title::text,                                           -- 2. title
  ...
  growth_rate_num::numeric as growth_rate_value,        -- 18. growth_rate_value (numeric)
