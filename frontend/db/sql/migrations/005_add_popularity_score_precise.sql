-- Migration 005: Add popularity_score_precise to Home Views
-- Date: 2025-10-21
-- Purpose: Expose popularity_score_precise column from news_trends in public_v_home_news and v_home_news
-- Risk: MEDIUM - Drops and recreates views (brief unavailability during transaction)
-- Idempotent: No - must drop views first
-- Rollback: Restore from migration 004 or previous backup

-- CONTEXT:
-- Frontend code expects popularity_score_precise (numeric) but views only exposed popularity_score.
-- Base table news_trends has both columns:
--   - popularity_score: numeric(6,3) - rounded display score (0-100)
--   - popularity_score_precise: numeric - full precision score for sorting
-- PostgreSQL won't let us add a column in the middle with CREATE OR REPLACE, so we must DROP CASCADE and recreate.
...
-- =====================================================
-- RECREATE public_v_home_news WITH popularity_score_precise
-- =====================================================
...
today_items AS (
    SELECT
        ...
        nt.popularity_score,
        nt.popularity_score_precise,  -- ADDED: Full precision score for sorting
...
SELECT
    ...
    ai_prompt,
    popularity_score,
    popularity_score_precise,  -- ADDED: Full precision score
...
'Home feed view with full precision popularity score for sorting.
Updated: 2025-10-21 to add popularity_score_precise column.
Security: INVOKER mode, read-only for anon/authenticated.
Contract: 28 columns (added popularity_score_precise).';

-- =====================================================
-- RECREATE v_home_news ALIAS TO PASS THROUGH NEW COLUMN
-- =====================================================
...
Updated: 2025-10-21 to pass through popularity_score_precise.
Contract: 28 columns (includes popularity_score_precise).';
...
BEGIN
    ...
    IF NOT has_precise_primary THEN
        RAISE EXCEPTION 'popularity_score_precise missing from public_v_home_news!';
    END IF;
    
    IF NOT has_precise_alias THEN
        RAISE EXCEPTION 'popularity_score_precise missing from v_home_news!';
    END IF;
    
    RAISE NOTICE 'Column popularity_score_precise exists in both views';
    ...
    RAISE NOTICE 'Migration 005 verification passed. popularity_score_precise added successfully.';
END $$;
